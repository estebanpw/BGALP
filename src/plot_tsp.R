...
nodes <- read.table("att48.tsp")

# Plot all nodes
plot(nodes[1,2], nodes[1,3], xlim = c(min(nodes[,2]), max(nodes[,2])), ylim = c(min(nodes[,3]), max(nodes[,3])), ylab = "Y-Cartesian points", xlab = "X-Cartesian points", main = "Partitions in ATT48 TSP")
for(i in 1:length(nodes[,1])){
  points(nodes[i,2], nodes[i,3])
}

# Plot solutions
rand1 <- c(16, 42, 29, 35, 17, 45, 32, 19, 11, 14, 39, 22, 10, 46, 20, 12, 24, 38, 31, 23, 44, 34, 3, 25, 9, 41, 1, 28, 4, 47, 13, 33, 40, 2, 21, 15, 0, 7, 8, 37, 30, 43, 6, 27, 5, 36, 18, 26)
...
for(i in 2:length(rand1)){
  lines(c(nodes[rand1[i-1],2], nodes[rand1[i],2]) , c(nodes[rand1[i-1],3], nodes[rand1[i],3]), col = "red")
}
lines(c(nodes[rand1[1],2], nodes[rand1[length(rand1)],2]) , c(nodes[rand1[1],3], nodes[rand1[length(rand1)],3]), col = "red")
